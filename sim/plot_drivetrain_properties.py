...
with open('drivetrain_data_left_2.json', 'r') as file:
    left_data = json.load(file)

with open('drivetrain_data_right_2.json', 'r') as file:
...
for i, prop in enumerate(properties):
    ...
    if prop in right_data:  # Check if property exists in right_data
        axes[i].plot(right_data["voltage"], right_data.get(prop, []), label=f"Right {prop.capitalize()}", color='orange')
    if prop in left_data_old:  # Check if property exists in left_data_old
        axes[i].plot(left_data_old["voltage"], left_data_old.get(prop, []), label=f"Left {prop.capitalize()} Old", color='blue', linestyle=':')
    if prop in right_data_old:  # Check if property exists in right_data_old
        axes[i].plot(right_data_old["voltage"], right_data_old.get(prop, []), label=f"Right {prop.capitalize()} Old", color='orange', linestyle=':')
    ...
    axes[i].legend(loc='upper left', framealpha=0.8)  # Set legend to top left with 75% transparency
    axes[i].grid(True)
