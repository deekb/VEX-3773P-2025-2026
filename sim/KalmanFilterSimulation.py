...
from sim.KalmanFilter import KalmanFilter
...
def main():
    ...
    # Kalman filter setup (3 state dims, 3 measurement dims)
    n = 3
    m = 3
    F = Matrix.identity(n)
    H_mat = Matrix.identity(n)
    ...
    kf = KalmanFilter(n=n, m=m, F=F, H=H_mat, Q=Q0, R=R0, P=P0, x0=[0.0, 0.0, 0.0])
