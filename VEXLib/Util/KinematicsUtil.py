...
class GenericWheel:
    ...
    def __init__(self, motor: Motor, direction, speed_controller: PIDController, wheel_circumference, motor_to_wheel_gear_ratio):
        """
        ...
        return Velocity1d.from_meters_per_second(
            self.speed_calculator.calculate_rate(
                arc_length_from_rotation(
                    ...
                    Rotation2d.from_revolutions(self.motor.position(TURNS)) * self.motor_to_wheel_gear_ratio,
                ).to_meters(),
                time.time(),
            )
        )
    ...
    def update(self):
        ...
        """
        self.motor.spin(
            FORWARD, self.speed_controller.update(self.get_wheel_speed().to_meters_per_second()), VOLT
        )

    def calculate_desired_wheel_speed(self, movement_angle: Rotation2d, movement_speed: Velocity1d) -> Velocity1d:
        """
        ...
        """
        return (
                movement_speed * self.wheel_contribution_coefficient(movement_angle.to_radians())
        )
...
class GenericDrivetrain:
    ...
    def move(self, movement_angle: Rotation2d, movement_speed: Velocity1d):
        ...
        for wheel in self.wheels:
            wheel.set_target_wheel_velocity(wheel.wheel_speed(movement_angle, movement_speed))
...
class GenericOdometry:
    ...
    def update(self, wheel_positions: list):
        ...
        distances = [
            current - last for current, last in zip(wheel_positions, self.last_wheel_positions)
        ]
        ...
        y_distance = Translation1d.from_meters(MathUtil.average_iterable([direction.sin() * distance.to_meters() for distance, direction in wheel_distances_and_directions]))
        x_distance = Translation1d.from_meters(MathUtil.average_iterable([direction.cos() * distance.to_meters() for distance, direction in wheel_distances_and_directions]))

        robot_relative_translation = Translation2d(x_distance, y_distance)

        self.pose.rotation = Rotation2d.from_degrees(
            self.inertial_sensor.rotation(DEGREES)
        ) - self.zero_rotation

        # Update the robot's 2D position based on forward distance and orientation
        self.pose.translation = self.pose.translation + robot_relative_translation.rotate_by(self.pose.rotation)
