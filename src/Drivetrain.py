...
class Drivetrain:
    def __init__(
        ...
        self.left_drivetrain_PID = PIDFController(
            DrivetrainProperties.LEFT_PIDF_GAINS, t=1e-5
        )
        self.right_drivetrain_PID = PIDFController(
            DrivetrainProperties.RIGHT_PIDF_GAINS, t=1e-5
        )
    ...
    def turn_to(self, rotation: Rotation2d):
        ...
        while (
            not self.rotation_PID.at_setpoint(
                threshold=self.TURNING_THRESHOLD.to_radians()
            )
        ) and (time.time() - start_time < DrivetrainProperties.TURN_TIMEOUT_SECONDS):
