...
for i, prop in enumerate(properties):
    ...
    if prop in left_data_old:  # Check if property exists in left_data_old
        axes[i].plot(left_data_old["voltage"], left_data_old.get(prop, []), label=f"Left {prop.capitalize()} Old",
                     color="blue", linestyle=":")
    if prop in right_data_old:  # Check if property exists in right_data_old
        axes[i].plot(right_data_old["voltage"], right_data_old.get(prop, []), label=f"Right {prop.capitalize()} Old",
    ...
    axes[i].legend(loc="upper left", framealpha=0.8)  # Set legend to top left with 80% transparency
    axes[i].grid(True)
