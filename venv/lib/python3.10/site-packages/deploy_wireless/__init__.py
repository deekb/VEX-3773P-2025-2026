...
def main():
    ...
    # Get dictionary of filenames and MD5 hashes from brain
    # For example
    libraries_on_brain = {
        "test.py": "32ab7a9c798de09f",
        "main.py": "32ab7a9c798de09a"
    }

    local_libraries = dict()

    for library in available_libraries:
        local_libraries[library] = get_checksum(library)

    libraries_to_copy = []

    for library in required_libraries:
        if library not in libraries_on_brain or required_libraries[library] != local_libraries[library]:
            libraries_to_copy.append(library)
