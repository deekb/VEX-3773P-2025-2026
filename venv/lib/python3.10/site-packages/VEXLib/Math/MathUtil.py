...
class MathUtil:
    def __init__(self):
        raise NotImplementedError("This is a utility class, please do not instantiate it")
    ...
    def average(*args) -> float:
        ...
        Examples:
            ...
            2.0
            >>> MathUtil.average(5, 10)
            7.5
            >>> MathUtil.average(3.5, 4.5, 6.0)
            4.666666666666667
            >>> MathUtil.average(1, 2, 3, 4, 5)
            3.0
        """
        return MathUtil.average_iterable(args)
    ...
    def input_modulus(input_, minimum_input, maximum_input):
        ...
        Returns:
            float: The wrapped value within the range [minimum_input, maximum_input).
        """
    ...
    def angle_modulus(cls, angle_radians):
        ...
        Returns:
            The wrapped angle.
        """
        angle = cls.input_modulus(angle_radians, -math.pi, math.pi)
        if angle == -math.pi:
            return math.pi
        return angle
    ...
    @classmethod
    def distance_from_point_to_line(cls, point, slope, y_intercept):
        """
        ...
        a, b, c = cls.slope_intercept_to_standard(slope, y_intercept)

        if math.isinf(slope):
            x_intercept = cls.standard_to_x_intercept(a, b, c)
            return abs(x0 - x_intercept)
        else:
            # Distance formula
            distance = abs(a * x0 + b * y0 + c) / cls.hypotenuse(a, b)
    ...
    def sin_accurate(x, terms=12):
        ...
        for n in range(terms):
            coefficient = (-1) ** n
            exponent = 2 * n + 1
            term = coefficient * (x ** exponent) / MathUtil.factorial(exponent)
