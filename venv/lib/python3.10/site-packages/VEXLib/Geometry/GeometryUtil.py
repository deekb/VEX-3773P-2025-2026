...
class GeometryUtil:
    ...
    def area_triangle(point1, point2, point3):
        ...
        """
        a = GeometryUtil.distance(point1, point2)
        b = GeometryUtil.distance(point2, point3)
        c = GeometryUtil.distance(point3, point1)
        s = (a + b + c) / 2
    ...
    def centroid(points):
        ...
        for i in range(len(points)):
            ...
            total_area += factor
            centroid_x += (points[i][0] + points[j][0]) * factor
            centroid_y += (points[i][1] + points[j][1]) * factor
    ...
    def intersection_point(line1_start, line1_end, line2_start, line2_end):
        ...
        if min(x1, x2) <= intersection_x <= max(x1, x2) and min(y1, y2) <= intersection_y <= max(y1, y2) \
                and min(x3, x4) <= intersection_x <= max(x3, x4) and min(y3, y4) <= intersection_y <= max(y3, y4):
    ...
    @staticmethod
    def distance_circle_rolled(circle_circumference: Distance, rotation):
